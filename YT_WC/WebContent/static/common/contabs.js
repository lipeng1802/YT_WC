...
//    });
    ...
    if(!isNew){
		    top.$('.J_menuTab').each(function () {
		    	...
		    	//tabID 去掉传递的参数
		    	var tabId = top.$(this).data('id');
		    	var subDataUrl=dataUrl;
		    	if(tabId.indexOf("?")>-1)tabId = tabId.substr(0,tabId.indexOf("?"));
		    	if(dataUrl.indexOf("?")>-1)subDataUrl = dataUrl.substr(0,dataUrl.indexOf("?"));
		    	
		    	console.log(tabId+"------"+dataUrl+"---------"+subDataUrl);
		        if (tabId == subDataUrl) {// 选项卡已存在，激活。
		            if (!top.$(this).hasClass('active')) {
		                ...
		                top.$('.J_mainContent .J_iframe').each(function () {
		                	var tabId2 = top.$(this).data('id');
		                	if(tabId2.indexOf("?")>-1)tabId2 = tabId2.substr(0,tabId2.indexOf("?"));
		                    if (tabId2 == subDataUrl) {
		                    	top.$(this).show().siblings('.J_iframe').hide();
		                    	/*如果匹配都带参数，添加自动刷新功能**/
		                    	if(dataUrl.indexOf("?")>-1 && top.$(this).data('id').indexOf("?")>-1){
		                    		top.$(this).attr("data-id",dataUrl).attr("src",dataUrl);
		                    	}
